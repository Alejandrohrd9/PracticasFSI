...
def search_branch_bound(problem):
    return graph_search_wClose(problem, MyList())

def branch_bound_underestimation(problem):
    return tree_search(problem, MyList_Underestimation(problem))
...
def graph_search_wClose(problem, fringe):
    fringe.append(Node(problem.initial))
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
